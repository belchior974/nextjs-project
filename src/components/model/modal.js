import { useState, useEffect } from "react";

import { Formik, Form } from "formik";

import Textfield from "../formsUI/Textfield";

import * as Yup from "yup";
import { BootstrapDialog, BootstrapDialogTitle, Transition } from "../modals/dialogs";

import { DialogContent, Grid, Button } from "@mui/material";

import toast from "react-hot-toast";

import axios from "axios";

export const ModelModal = (props) => {
  const { model, setOpenModal, refetch, setRefetch } = props;

  const [open, setOpen] = useState(false);

  const [formValues, setFormValues] = useState({
    brand: {
      id: "",
    },
    description: "",
    color: "",
    description: "",
    doors: 0,
    engine_power: "",
  });

  const formValidation = Yup.object().shape({
    description: Yup.string().max(100).required("Campo obrigatório"),
  });

  useEffect(() => {
    if (model) {
      edit();
    }
    setOpen(true);
  }, [setOpenModal]);

  const handleClose = async () => {
    await setOpen(false);
    setTimeout(() => {
      props.setOpenModal(false);
    }, 1000);
  };
  const onSubmit = async (values, { resetForm }) => {
    if (model) {
      update(values, { resetForm });
    } else {
      create(values, { resetForm });
    }
  };

  const create = async (values, { resetForm }) => {
    try {
      axios
        .post("http://localhost:8081/model", {
          brand: {
            id: Number(values.brand.id),
            description: "",
          },
          description: values.description,
          color: values.color,
          doors: values.doors,
          engine_power: values.engine_power,
        })
        .then(async (response) => {
          console.log(response);
          await setRefetch(true);
          toast.success("Operação realizada com sucesso!");
          handleClose();
          resetForm();
        });
    } catch (e) {
      console.error(e);
      toast.error("Algo deu errado!");
    }
  };

  const update = async (values, { resetForm }) => {
    try {
      axios
        .put("http://localhost:8081/model", {
          id: values.id,
          brand: {
            id: Number(values.brand.id),
            description: "",
          },
          description: values.description,
          color: values.color,
          doors: values.doors,
          engine_power: values.engine_power,
        })
        .then(async (response) => {
          await setRefetch(true);
          toast.success("Operação realizada com sucesso!");
          handleClose();
          resetForm();
        });
    } catch (e) {
      console.error(e);
      toast.error("Algo deu errado!");
    }
  };

  const edit = () => {
    console.log(model);

    setFormValues(model);
  };

  useEffect(() => {
    console.log(formValues);
  }, [formValues]);

  return (
    <>
      <BootstrapDialog
        onClose={handleClose}
        open={open}
        TransitionComponent={Transition}
        fullWidth={true}
        maxWidth="md"
      >
        <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
          {model?.id ? "Atualização" : "Cadastro "} de Marca {model?.id ? `[${model.id}]` : ""}
        </BootstrapDialogTitle>
        <DialogContent>
          <>
            <Formik
              initialValues={{ ...formValues }}
              validationSchema={formValidation}
              enableReinitialize
              onSubmit={onSubmit}
            >
              {({ values }) => (
                <Form>
                  <Grid container spacing={2} sx={{ pt: 2 }}>
                    <Grid item xs={4} sx={{ mt: 0.5 }}>
                      <Textfield autoFocus label="Marca" name="brand.id" type="text" />
                    </Grid>

                    <Grid item xs={8} sx={{ mt: 0.5 }}>
                      <Textfield label="Descrição" name="description" type="text" />
                    </Grid>

                    <Grid item xs={3} sx={{ mt: 0.5 }}>
                      <Textfield label="Cor" name="color" type="text" />
                    </Grid>

                    <Grid item xs={3} sx={{ mt: 0.5 }}>
                      <Textfield label="Quantidade Portas" name="doors" type="number" />
                    </Grid>

                    <Grid item xs={3} sx={{ mt: 0.5 }}>
                      <Textfield label="Motor" name="engine_power" type="text" />
                    </Grid>

                    <Grid item xs={12}>
                      <Button type="submit" size="large" fullWidth variant="contained">
                        {model != null ? "Atualizar" : "Salvar"}
                      </Button>
                    </Grid>
                  </Grid>
                </Form>
              )}
            </Formik>
          </>
        </DialogContent>
      </BootstrapDialog>
    </>
  );
};
